...
!function() {
	...
	function e(p) {
		return p.replace(/&amp;/g, "%26").replace(/&/g, "%26").replace(/=/g,
				"%3D")
	}
	function f() {
		...
		j(
				this,
				{
					...
					_fireEvent : function(w, z, x, B) {
						...
						if (z
								&& "onBeforeBegin,onMetaData,onMetaDataChange,onStart,onUpdate,onResume"
										.indexOf(w) != -1) {
							j(B, z);
							if (z.metaData) {
								if (!B.duration) {
					...
					if (!v) {
						n(A.methods, function() {
							...
							p[C] = function() {
								...
								return E === "undefined" || E === undefined ? p
										: E
							}
						});
						v = true
	...
	var g = i(
			window.flashembed,
			{
				...
				getVersion : function() {
					...
					f = e.exec(f);
					return f ? [ 1 * f[1], 1 * f[(f[1] * 1 > 9 ? 2 : 3)] * 1 ]
							: [ 0, 0 ]
				},
				asString : function(l) {
